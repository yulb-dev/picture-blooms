...
class GoodsCard extends Component {
    ...
    render() {
        let date = new Date(this.state.ctime)
        let color = ['#36a08a', '#5db4c7', '#e52e2e']
        return (
            <div className='GoodsCard pattern-diagonal-lines-lg'>
                <div className="imgBox">
                    <img src={this.state.imgsrc} alt='img' ref={this.Myimg} onLoad={this.imgIsOk.bind(this)} />
                </div>
                <div className='cardLabels'>{this.state.labels.map((item, index) => <span key={index} style={{ backgroundColor: color[index] }}>{item}</span>)}{`${date.getFullYear()}-${date.getMonth() + 1}-${date.getDate()}`}</div>
                <h1 className='cardTitle'>{this.state.title}</h1>
                <p className='cardContent'>{this.state.content}</p>
