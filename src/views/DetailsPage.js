...
import { addComment } from '../network/details'
...
class DetailsPage extends Component {
    constructor(props) {
        ...
        if (!this.props.location.query) {
            ...
            const { cardId, userId } = this.props.location.query
            getDynamic(cardId).then((data) => {
                getUserMessage(userId).then((data2) => {
                    this.setState({ ...data, ...data2, cardId })
                })
            })
        }
    ...
    render() {
        if (this.props.location.query) {
            if (this.state.labels) {
                const { value, useravatar, username, ctime, title, imgsrc, labels, content, likesnum, comment, isShow, isborder, userid } = this.state
