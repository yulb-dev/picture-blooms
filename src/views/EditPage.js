...
import { goEdit } from '../network/registered'
...
class Edit extends Component {
                    ...
                    <div className='AddPage'>
                        ...
                        <div className='topimg'>
                            <input type="file" id="file"
                                onChange={this.inputChange.bind(this)} name="userAvatar"
    ...
    publishAnArticle() {
        const { title, file, labels, content, _id } = this.state
        if (labels.length < 1) {
            alert('添加标签！')
            return
        }
        ...
        data.append('cardid', _id)
        goEdit(data).then((data) => {
            if (data.keyValue) {
                throw (data)
            }
            else {
